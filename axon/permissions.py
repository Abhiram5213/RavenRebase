...
def check_app_permission():
	...
	if frappe.db.exists("Axon User", {"user": frappe.session.user}):
		return True

	return False


def axon_user_has_permission(doc, user=None, ptype=None):

	if doc.type == "Bot":
		# Anyone with Axon User role can view the bot details
		if ptype == "read":
			return True

		# Axon Admins can change the bot details
		roles = frappe.get_roles(user)
		if "Axon Admin" in roles:
			return True
	else:
...
def workspace_has_permission(doc, user=None, ptype=None):
	"""
	1. Only Axon Admins can create a workspace
	...
	"""
	if ptype == "create":
		# Return True since this is checked via the Role in the doctype - only Axon Admins can create a workspace
		return True
...
def workspace_member_has_permission(doc, user=None, ptype=None):
	...
	if ptype == "create":
		workspace_type = frappe.get_cached_value("Axon Workspace", doc.workspace, "type")
...
def channel_has_permission(doc, user=None, ptype=None):
	if doc.is_direct_message:
		...
		if ptype == "create":
			...
			main_channel = frappe.get_cached_value("Axon Message", doc.channel_name, "channel_id")
		...
		if ptype == "read":
			...
			channel_doc = frappe.get_cached_doc("Axon Channel", main_channel)
		...
		if ptype == "create":
			...
			if not frappe.db.get_value("Axon Workspace", doc.workspace, "only_admins_can_create_channels"):
				return True
		...
		if ptype == "delete" or ptype == "write":
			...
			if channel_member:
				if channel_member.get("is_admin"):
					return True
				# If the user is a Axon Admin, they can update or delete the channel
				roles = frappe.get_roles(user)
				if "Axon Admin" in roles:
					return True
	...
	# 	if doc.workspace:
	# 		return frappe.db.exists("Axon Workspace Member", {"workspace": doc.workspace, "user": user})
	# 	return True
	# elif doc.type == "Private":
	# 	if doc.is_thread:
	# 		if ptype == "read" or ptype == "create":
	# 			# Only users who are part of the original channel can read the thread
	# 			return frappe.has_permission(doctype="Axon Message", doc=doc.name, ptype="read")

	# 	if frappe.db.exists("Axon Channel Member", {"channel_id": doc.name, "user_id": user}):
	# 		return True
	# 	elif (
	# 		doc.owner == user and frappe.db.count("Axon Channel Member", {"channel_id": doc.name}) <= 0
	# 	):
	# 		return True
	# 	else:
	# 		return False
...
def channel_member_has_permission(doc, user=None, ptype=None):
	...
	if ptype == "read":
		...
		return frappe.db.exists("Axon Channel Member", {"channel_id": doc.channel_id, "user_id": user})

	channel_doc = frappe.get_cached_doc("Axon Channel", doc.channel_id)
	...
	if ptype == "create":
		if is_direct_message:
			...
			if channel_doc.is_thread:
				...
				main_channel = frappe.db.get_value("Axon Message", doc.channel_id, "channel_id")
			...
			if is_workspace_member(workspace, doc.user_id):
				return frappe.db.exists(
					"Axon Channel Member", {"channel_id": doc.channel_id, "user_id": user}
				)
	...
	if ptype == "write":
		...
		if channel_member:
			if channel_member.get("is_admin"):
				return True
			# If the user is a Axon Admin, they can update the channel member
			roles = frappe.get_roles(user)
			if "Axon Admin" in roles:
				return True
	...
	# 	if frappe.db.exists("Axon Channel Member", {"channel_id": doc.channel_id, "user_id": user}):
	# 		return True
...
def axon_poll_vote_has_permission(doc, user=None, ptype=None):
	"""
	...
	if not frappe.has_permission("Axon Poll", doc=doc.poll_id, ptype="read", user=user):
		return False

	if ptype == "create":
		# User can only vote if they are a member of the channel
		channel_id = frappe.get_cached_value("Axon Message", {"poll_id": doc.poll_id}, "channel_id")
	...
	if ptype in ["read", "delete"]:
		if doc.owner == user:
			...
			is_anonymous = frappe.get_cached_value("Axon Poll", doc.poll_id, "is_anonymous")
			if not is_anonymous:
				if ptype == "read":
					return True
...
def axon_poll_has_permission(doc, user=None, ptype=None):
	"""
	...
	if ptype == "read" or ptype == "delete" or ptype == "write":
		...
		message = frappe.get_doc("Axon Message", {"poll_id": doc.name})
...
def axon_workspace_query(user):
	if not user:
		user = frappe.session.user

	# Get all workspaces that the user is a member of
	workspace_members = frappe.get_all(
		"Axon Workspace Member", filters={"user": user}, fields=["workspace"]
	)

	workspace_names = [frappe.db.escape(member.workspace) for member in workspace_members]

	if workspace_names:
		return f"`tabAxon Workspace`.name in ({', '.join(workspace_names)}) OR `tabAxon Workspace`.type = 'Public'"
	else:
		return "`tabAxon Workspace`.type = 'Public'"


def axon_workspace_member_query(user):
	if not user:
		user = frappe.session.user
	...
	if workspace_names:
		return f"`tabAxon Workspace Member`.workspace in ({', '.join(workspace_names)})"
	else:
		return f"`tabAxon Workspace Member`.owner = {frappe.db.escape(user)}"


def axon_channel_member_query(user):
	if not user:
		user = frappe.session.user

	return f"`tabAxon Channel Member`.user_id = {frappe.db.escape(user)}"


def axon_channel_query(user):
	if not user:
		user = frappe.session.user
	...
	"""
      ...
      We could also remove "Axon User" from the Axon Channel doctype role, but then permission checks for joining socket rooms for the channel would fail

      Hence, we are adding a WHERE clause to the query - this is inconsequential since we will never use the standard get_list query for Axon Channel,
      but needed for security since we do not want users to be able to view channels they are not a member of
    """
	return f"`tabAxon Channel`.owner = {frappe.db.escape(user)}"


def axon_message_query(user):
	if not user:
		user = frappe.session.user
	...
	"""
      ...
      We could also remove "Axon User" from the Axon Message doctype role, but then permission checks for attached files would also fail.

      Hence, we are adding a WHERE clause to the query - this is inconsequential since we will never use the standard get_list query for Axon Message,
      but needed for security since we do not want users to be able to view messages from channels they are not a member of
    """
	return f"`tabAxon Message`.owner = {frappe.db.escape(user)}"


def axon_poll_query(user):
	if not user:
		user = frappe.session.user

	return f"`tabAxon Poll`.owner = {frappe.db.escape(user)}"


def axon_poll_vote_query(user):
	if not user:
		user = frappe.session.user
	...
	"""
	  ...
	  Hence, we are adding a WHERE clause to the query - this is inconsequential since we will never use the standard get_list query for Axon Poll Vote,
	  but needed for security since we do not want users to be able to view votes from polls they did not vote for
	"""
	return f"`tabAxon Poll Vote`.owner = {frappe.db.escape(user)}"
