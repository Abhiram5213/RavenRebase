...
from axon.utils import get_channel_members
...
def send_notification_for_message(message):
	...
	if axon_settings.push_notification_service == "Axon":
		send_push_notification_via_axon_cloud(message, axon_settings)

		return

	channel_doc = frappe.get_cached_doc("Axon Channel", message.channel_id)
...
def send_push_notification_via_axon_cloud(message, axon_settings):
	"""
	Send a push notification via the Axon Cloud API
	"""
	channel_doc = frappe.get_cached_doc("Axon Channel", message.channel_id)
	...
	try:
		...
		data = {
			...
			"axon_message_type": message.message_type,
		...
		make_post_call_for_notification(messages, axon_settings)

	except Exception as e:
		frappe.log_error(title="Axon Cloud Push Notification Error")


def make_post_call_for_notification(messages, axon_settings):
	"""
	...
	client.post_api(
		"axon_cloud.api.notification.send_to_users",
		params={
...
def send_notification_to_user(user_id, title, message, data=None, user_image_path=None):
	...
	try:
		...
		push_notification = PushNotification("axon")
		...
		if push_notification.is_enabled():
			...
			if data.get("channel_id"):
				link = frappe.utils.get_url() + "/axon/channel/" + data.get("channel_id", "")
...
def subscribe_user_to_topic(channel_id, user_id):
	...
	"""
	notification_service = frappe.db.get_single_value("Axon Settings", "push_notification_service")

	if notification_service == "Axon":
		return
