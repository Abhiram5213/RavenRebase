...
from axon.ai.openai_client import (
	code_interpreter_file_types,
	file_search_file_types,
	get_open_ai_client,
)
from axon.utils import get_axon_user


class AxonBot(Document):
	...
	if TYPE_CHECKING:
		...
		from axon.axon_ai.doctype.axon_ai_bot_files.axon_ai_bot_files import AxonAIBotFiles
		from axon.axon_ai.doctype.axon_bot_functions.axon_bot_functions import AxonBotFunctions

		allow_bot_to_write_documents: DF.Check
		bot_functions: DF.Table[AxonBotFunctions]
		bot_name: DF.Data
		debug_mode: DF.Check
		...
		file_sources: DF.Table[AxonAIBotFiles]
		...
		if not self.allow_bot_to_write_documents:
			for f in self.bot_functions:
				needs_write = frappe.db.get_value(
					"Axon AI Function", f.function, "requires_write_permissions"
				)
				if needs_write:
					frappe.throw(
	...
	def on_update(self):
		"""
		When a bot is updated, create/update the Axon User for it

		TODO: Generate JSON files when a Standard Bot is created or updated
		"""
		if self.axon_user:
			axon_user = frappe.get_doc("Axon User", self.axon_user)
			axon_user.type = "Bot"
			axon_user.bot = self.name
			axon_user.full_name = self.bot_name
			axon_user.first_name = self.bot_name
			axon_user.user_image = self.image
			axon_user.enabled = 1
			axon_user.save()
		else:
			...
			self.db_set("axon_user", axon_user.name)
	...
	def on_trash(self):
		...
		if self.axon_user:
			frappe.db.set_value("Axon User", self.axon_user, "bot", None)
			self.db_set("axon_user", None)
			frappe.delete_doc("Axon User", self.axon_user)
	...
	def update_openai_assistant(self):
		...
		# Additional check because it is being used in Axon AI Function
	...
	def check_and_update_enabled_tools(self, assistant):
		...
		try:
			...
			frappe.log_error(
				f"Axon AI Bot Tool Check Error for {self.name}",
				str(e),
			)
	...
	def get_tools_for_assistant(self):
		...
		for f in self.bot_functions:
			function_def = frappe.db.get_value("Axon AI Function", f.function, "function_definition")
			if function_def:
	...
	def get_tool_resources_for_assistant(self):
		...
		# Join file sources with Axon AI Bot Files
	...
	def is_member(self, channel_id: str) -> None | str:
		...
		member_id = frappe.db.exists(
			"Axon Channel Member", {"channel_id": channel_id, "user_id": self.axon_user}
		)
		if member_id:
			return member_id
		return None
	...
	def add_to_channel(self, channel_id: str) -> str:
		...
		if not existing_member:
			axon_channel_member = frappe.get_doc(
				doctype="Axon Channel Member", user_id=self.axon_user, channel_id=channel_id
			)
			axon_channel_member.insert()
			return axon_channel_member.name
		else:
	...
	def remove_from_channel(self, channel_id: str) -> None:
		...
		existing_member = self.is_member(channel_id)
		if existing_member:
			frappe.delete_doc("Axon Channel Member", existing_member)

	def get_dm_channel_id(self, user_id: str) -> str | None:
		"""
		Get the channel_id of a Direct Message channel with a user
		"""
		# The User's Axon User ID
		user_axon_user = frappe.db.get_value("Axon User", {"user": user_id}, "name")
		if not user_axon_user:
			return None

		# get canonical order of the users
		if self.axon_user > user_axon_user:
			dm_user_1, dm_user_2 = self.axon_user, user_axon_user
		else:
	...
	def create_direct_message_channel(self, user_id: str) -> str:
		...
		Throws an error if the user is not a Axon User
		"""
		...
		if channel_id:
			...
			# The user's axon_user document
			user_axon_user = get_axon_user(user_id)
			if not user_axon_user:
				frappe.throw(f"User {user_id} is not added as a Axon User")
	...
	def get_last_message(self, channel_id: str = None, message_type: str = None) -> Document | None:
		...
		return frappe.get_last_doc("Axon Message", filters=filters, order_by="creation desc")
	...
	def get_previous_messages(
		...
		return frappe.get_all("Axon Message", filters=filters, order_by="creation desc", pluck="name")
