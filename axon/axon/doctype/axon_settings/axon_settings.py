...
class AxonSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from axon.axon_integrations.doctype.axon_hr_company_workspace.axon_hr_company_workspace import (
			AxonHRCompanyWorkspace,
		)

		auto_add_system_users: DF.Check
		auto_create_department_channel: DF.Check
		company_workspace_mapping: DF.Table[AxonHRCompanyWorkspace]
		...
		enable_video_calling_via_jitsi: DF.Check
		enable_video_calling_via_livekit: DF.Check
		jitsi_server_url: DF.Data | None
		...
		push_notification_service: DF.Literal["None", "Frappe Cloud", "Axon"]
		show_if_a_user_is_on_leave: DF.Check
		show_axon_on_desk: DF.Check
	...
	def validate(self):
		...
		if self.push_notification_service == "Axon":
