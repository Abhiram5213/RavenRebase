...
from axon.ai.agents_integration import handle_ai_request_sync
from axon.ai.google_ai import run_document_ai_processor

# Keep old handler import for fallback
from axon.ai.handler import stream_response
...
def handle_ai_thread_message(message, channel):
	...
	bot = frappe.get_cached_doc("Axon Bot", channel.thread_bot)
...
def handle_ai_thread_message_with_assistants(message, channel, bot):
	...
	if message.message_type in ["File", "Image"]:
		...
		try:
			...
			frappe.log_error("Axon AI Error", frappe.get_traceback())
			bot.send_message(
				channel_id=channel.name,
...
def process_message_with_agent(
	...
	from axon.ai.conversation_file_handler import ConversationFileHandler
