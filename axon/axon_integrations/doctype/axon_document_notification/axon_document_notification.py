...
FORBIDDEN_DOCUMENT_TYPES = frozenset("Axon Message")


class AxonDocumentNotification(Document):
	...
	if TYPE_CHECKING:
		...
		from axon.axon_integrations.doctype.axon_document_notification_recipients.axon_document_notification_recipients import (
			AxonDocumentNotificationRecipients,
		)
		...
		recipients: DF.Table[AxonDocumentNotificationRecipients]
	...
	def validate(self):
		...
		frappe.cache().hdel("axon_doc_notifications", self.document_type)
		...
		for recipient in self.recipients:
			if recipient.channel_type == "User":
				if recipient.variable_type == "Static":
					# Check if this user exists in Axon
					if not frappe.db.exists("Axon User", recipient.value):
						frappe.throw(_("User {0} does not exist in Axon.").format(recipient.value))

			elif recipient.channel_type == "Channel":
				if recipient.variable_type == "Static":
					# Check if this channel exists in Axon
					if not frappe.db.exists("Axon Channel", recipient.value):
						frappe.throw(_("Channel {0} does not exist in Axon.").format(recipient.value))
	...
	def send_notification(self, context, link_doctype, link_document):
		...
		bot = frappe.get_doc("Axon Bot", self.sender)
...
def run_document_notification(doc, method):
	...
	Enqueues the notifications to be sent on Axon
	"""
	...
	axon_notifications = frappe.cache().hget(
		"axon_doc_notifications", doc.doctype, _get_notifications
	)

	if not axon_notifications:
	...
	for notification in axon_notifications:
...
def evaluate_condition(context, condition, notification_name):
	...
	try:
		...
		message = _("Error while evaluating Axon Notification {0}. Please fix your template.").format(
			frappe.utils.get_link_to_form("Axon Document Notification", notification_name)
		)
		frappe.throw(message, title=_("Error in Axon Notification"))
		...
		frappe.log_error(
			title=title,
			message=message,
			reference_doctype="Axon Document Notification",
			reference_name=notification_name,
		)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		frappe.throw(msg, title=_("Error in Axon Notification"))


def send_axon_notifications(doc, notifications_to_send, link_doctype, link_document):
	"""
	Background job to send notifications to Axon
	"""
	context = get_context(doc)
	for notification in notifications_to_send:
		notification_doc = frappe.get_doc("Axon Document Notification", notification.name)
