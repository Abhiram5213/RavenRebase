...
const ChannelList = ({ onSelectChannel }) => {
    ...
    return (
        <div className="axon-channel-list">
...
const SkeletonLoader = () => {
    ...
    return <>{Array.from({ length: 17 }).map((_, i) => <div key={i} className="axon-channel-list-item">
...
const ChannelListItem = ({ channel, onClick }) => {
    ...
    return <button className="axon-channel-list-item" onClick={onClick}>
        <div className='channel-label'>
            <span className='axon-channel-icon'>
                <ChannelIcon channelType={channel.channel_type} />
            </span>
            <span className="axon-channel-list-item__name">{channel.channel_name}</span>
        </div>
        <div>
...
const DMChannelListItem = ({ channel, onClick }) => {
    ...
    return <button className="axon-channel-list-item" onClick={onClick}>
        <div className='channel-label'>
            <Avatar user={user} />
            <span className="axon-channel-list-item__name">{user?.full_name}</span>
        </div>
        <div>
            {count ? <span className="channel-unread-count">{count}</span> : null}
        </div>
