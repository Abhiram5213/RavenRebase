...
from axon.utils import delete_channel_members_cache, delete_workspace_members_cache
...
def get_list():
	...
	"""
	if not frappe.db.exists("Axon User", {"user": frappe.session.user}):
		frappe.throw(_("You do not have access to Axon."), frappe.PermissionError)

	workspace = frappe.qb.DocType("Axon Workspace")
	workspace_member = frappe.qb.DocType("Axon Workspace Member")
...
def join_workspace(workspace: str):
	...
	member = frappe.get_doc(
		{"doctype": "Axon Workspace Member", "workspace": workspace, "user": frappe.session.user}
	).insert()
...
def leave_workspace(workspace: str):
	...
	member = frappe.db.exists(
		"Axon Workspace Member", {"workspace": workspace, "user": frappe.session.user}
	)
	if member:
		member_doc = frappe.get_doc("Axon Workspace Member", member)
		member_doc.delete()
...
def can_create_channel(workspace: str):
	...
	"""
	workspace_doc = frappe.get_doc("Axon Workspace", workspace)
...
def fetch_workspace_members(workspace: str):
	...
	"""
	frappe.has_permission("Axon Workspace", doc=workspace, throw=True)
...
def get_workspace_member_count(workspace: str):
	...
	"""
	frappe.has_permission("Axon Workspace", doc=workspace, throw=True)
	return frappe.db.count("Axon Workspace Member", filters={"workspace": workspace})
...
def add_workspace_members(workspace: str, members: list):
	...
	"""
	frappe.has_permission("Axon Workspace", doc=workspace, ptype="write", throw=True)
	...
	for member in members:
		member_doc = frappe.get_doc(
			{"doctype": "Axon Workspace Member", "workspace": workspace, "user": member}
		)
...
def update_workspace_members(workspace: str, members: list):
	...
	for member in members:
		try:
			...
			member_id = frappe.db.exists("Axon Workspace Member", {"workspace": workspace, "user": user})

			# If not a member, delete the member
			if not is_member:
				if member_id:
					member_doc = frappe.get_doc("Axon Workspace Member", member_id)
					member_doc.flags.ignore_cache_invalidation = True
	...
	delete_workspace_members_cache(workspace)
	workspace_channels = frappe.get_all("Axon Channel", {"workspace": workspace}, pluck="name")
