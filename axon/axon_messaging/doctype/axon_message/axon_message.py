...
from axon.ai.ai import handle_ai_thread_message, handle_bot_dm
from axon.api.axon_channel import get_peer_user
from axon.notification import (
...
class AxonMessage(Document):
	...
	if TYPE_CHECKING:
		...
		from axon.axon_messaging.doctype.axon_mention.axon_mention import AxonMention
	...
	def validate_linked_message(self):
		...
		if self.linked_message:
			if (
				frappe.get_cached_value("Axon Message", self.linked_message, "channel_id") != self.channel_id
			):
	...
	def handle_ai_message(self):
		...
		channel_doc = frappe.get_cached_doc("Axon Channel", self.channel_id)
		...
		peer_user_doc = frappe.get_cached_doc("Axon User", peer_user.get("user_id"))

		if peer_user_doc.type != "Bot" or not peer_user_doc.bot:
			return

		bot = frappe.get_cached_doc("Axon Bot", peer_user_doc.bot)
	...
	def set_last_message_timestamp(self):
		...
		axon_channel = frappe.qb.DocType("Axon Channel")
		query = (
			frappe.qb.update(axon_channel)
			.where(axon_channel.name == self.channel_id)
			.set(axon_channel.last_message_timestamp, self.creation)
			.set(axon_channel.last_message_details, message_details)
		)
		query.run()
		...
		if channel_doc.is_direct_message:
			...
			if not channel_doc.is_self_message:
				...
				if peer_user_doc.get("type") == "User":
					...
					frappe.publish_realtime(
						"axon:unread_channel_count_updated",
						{
							"channel_id": self.channel_id,
							"play_sound": True,
							"sent_by": self.owner,
			...
			# This event needs to be published to all users on Axon (desk + website)
			frappe.publish_realtime(
				"axon:unread_channel_count_updated",
				{
					"channel_id": self.channel_id,
	...
	def add_mentioned_users_to_thread(self):
		...
		parent_channel_id = frappe.get_cached_value("Axon Message", self.channel_id, "channel_id")
		if parent_channel_id:
			for mention in self.mentions:
				if is_channel_member(parent_channel_id, mention.user) and not is_channel_member(
					...
					try:
						frappe.get_doc(
							{"doctype": "Axon Channel Member", "channel_id": self.channel_id, "user_id": mention.user}
	...
	def get_message_owner_details(self):
		...
		"""
		if self.is_bot_message:
			doc = frappe.get_cached_doc("Axon User", self.bot)
			return doc.full_name, doc.user_image
		else:
			doc = frappe.get_cached_doc("Axon User", self.owner)
			return doc.full_name, doc.user_image
	...
	def send_notification_for_direct_message(self):
		...
		"""
		peer_axon_user = frappe.db.get_value(
			"Axon Channel Member",
			{"channel_id": self.channel_id, "user_id": ("!=", self.owner)},
			"user_id",
		)

		if not peer_axon_user:
			return

		peer_axon_user_doc = frappe.get_cached_doc("Axon User", peer_axon_user)

		# Do not send notification to a bot
		if peer_axon_user_doc.type == "Bot":
			return
		...
		send_notification_to_user(
			user_id=peer_axon_user_doc.user,
			...
			data={
				"message_id": self.name,
				"channel_id": self.channel_id,
				"axon_message_type": self.message_type,
				"channel_type": "DM",
				"content": content,
	...
	def send_notification_for_channel_message(self):
		...
		is_thread = frappe.get_cached_value("Axon Channel", self.channel_id, "is_thread")
		...
		if is_thread:
			...
			channel_name = frappe.get_cached_value("Axon Channel", self.channel_id, "channel_name")
	...
	def after_delete(self):
		...
		if self.message_type == "Poll":
			frappe.delete_doc("Axon Poll", self.poll_id, ignore_permissions=True, delete_permanently=True)
	...
	def on_trash(self):
		...
		frappe.db.delete("Axon Message Reaction", {"message": self.name})
		...
		if self.is_thread:
			...
			thread_channel_doc = frappe.get_doc("Axon Channel", self.name)
			thread_channel_doc.delete(ignore_permissions=True)

		# delete the pinned message
		is_pinned = frappe.get_all(
			"Axon Pinned Messages", {"message_id": self.name, "parent": self.channel_id}
		)
		if is_pinned:
			channel_doc = frappe.get_doc("Axon Channel", self.channel_id)
			pinned_row = None
...
def on_doctype_update():
	"""
	Add indexes to Axon Message table
	"""
	# Index the selector (channel or message type) first for faster queries (less rows to sort in the next step)
	frappe.db.add_index("Axon Message", ["channel_id", "creation"])
	frappe.db.add_index("Axon Message", ["message_type", "creation"])
