...
def add_token_to_axon_cloud(user_id, token):
	"""
	Add a token to Axon Cloud
	"""
	...
	client.post_api(
		"axon_cloud.api.notification.create_user_token",
		params={
			"user_id": user_id,
			"token": token,
...
def delete_token_from_axon_cloud(user_id, token):
	"""
	Delete a token from Axon Cloud
	"""
	axon_settings = frappe.get_single("Axon Settings")
	...
	client.post_api(
		"axon_cloud.api.notification.delete_user_token",
		params={
			"user_id": user_id,
			"token": token,
			"site_name": get_site_name(),
...
def sync_users_tokens_to_axon_cloud():
	"""
	Sync all the tokens available on this site to Axon Cloud
	"""
	axon_settings = frappe.get_single("Axon Settings")
	tokens = frappe.db.get_all("Axon Push Token", fields=["user", "fcm_token"], order_by="user")
	...
	for chunk in chunks:
		...
		client.post_api(
			"axon_cloud.api.notification.import_user_tokens",
			params={
				"site_name": get_site_name(),
