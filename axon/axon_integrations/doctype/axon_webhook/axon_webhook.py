...
class AxonWebhook(Document):
	...
	def before_insert(self):
		...
		webhook = frappe.get_all("Axon Webhook", filters={"name": self.name})
		if webhook:
	...
	def get_doctype_and_event(self):
		doctypes_and_events = [
			...
			{"label": "Message Edited", "doctype": "Axon Message", "event": "on_update"},
			{"label": "Message Deleted", "doctype": "Axon Message", "event": "on_trash"},
			{"label": "Message Reacted On", "doctype": "Axon Message Reaction", "event": "after_insert"},
			{"label": "Channel Created", "doctype": "Axon Channel", "event": "after_insert"},
			{"label": "Channel Deleted", "doctype": "Axon Channel", "event": "on_trash"},
			{
			...
			{"label": "User Added", "doctype": "Axon User", "event": "after_insert"},
			{"label": "User Deleted", "doctype": "Axon User", "event": "on_trash"},
		]
	...
	def get_conditions(self):
		...
		if self.trigger_webhook_on_condition:
			if self.conditions_on == "Channel":
				if doctype == "Axon Channel":
					# return 'doc.name == self.channel_id'
					return f'doc.name == "{self.channel_id}"'
				elif doctype == "Axon Channel Member":
					return f'doc.channel_id == "{self.channel_id}"'
				elif doctype == "Axon Message":
					return f'doc.channel_id == "{self.channel_id}"'
				elif doctype == "Axon Message Reaction":
					frappe.throw(_("Message Reaction cannot be triggered on Channel"))
				elif doctype == "Axon User":
					frappe.throw(_("Axon User cannot be triggered on Channel"))
