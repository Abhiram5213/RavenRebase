...
class AxonWorkspaceMember(Document):
	...
	def validate(self):
		...
		if frappe.db.exists("Axon Workspace Member", conditions):
	...
	def delete_channel_members_for_user(self):
		...
		"""
		workspace_channels = frappe.get_all("Axon Channel", {"workspace": self.workspace}, pluck="name")
		for channel in workspace_channels:
			frappe.db.delete("Axon Channel Member", {"channel_id": channel, "user_id": self.user})
...
def on_doctype_update():
	"""
	Add indexes to Axon Workspace Member table
	"""
	# Index the selector first for faster queries (less rows to sort in the next step)
	frappe.db.add_index("Axon Workspace Member", ["workspace", "user"])
