...
from axon.utils import track_channel_visit
...
def get_messages(channel_id: str, limit: int = 20, base_message: str | None = None):
	...
	if base_message and frappe.db.exists("Axon Message", base_message):
...
def get_messages_around_base(channel_id: str, base_message: str):
	...
	from_timestamp = frappe.get_cached_value("Axon Message", base_message, "creation")
...
def get_older_messages(channel_id: str, from_message: str, limit: int = 20):
	...
	if not frappe.has_permission(doctype="Axon Channel", doc=channel_id, ptype="read"):
		frappe.throw(_("You do not have permission to access this channel"), frappe.PermissionError)
	# Fetch older messages for the channel
	from_timestamp = frappe.get_cached_value("Axon Message", from_message, "creation")
