"""
Improved agents integration with better import handling based on Desktop Axon approach.
"""
...
class AxonAgentManager:
	"""Manages Axon agents with local LLM/OpenAI support"""

	def __init__(self, bot_doc, file_handler=None):
		self.bot_doc = bot_doc
		self.settings = frappe.get_single("Axon Settings")
	...
	def _setup_client(self):
		...
		if self.bot_doc.model_provider == "Local LLM" and self.settings.enable_local_llm:
			...
			if not self.settings.local_llm_api_url:
				frappe.throw(_("Local LLM API URL is not configured in Axon Settings"))
			...
			if not api_key:
				frappe.throw(_("OpenAI API key is not configured in Axon Settings"))
	...
	def _setup_tools(self):
		...
		try:
			from axon.ai.sdk_tools import create_axon_tools

			axon_tools = create_axon_tools(self.bot_doc)
			if axon_tools:
				self.tools.extend(axon_tools)
		except Exception as e:
			frappe.log_error(
				f"Error loading Axon AI Functions: {str(e)}\n{traceback.format_exc()}",
				"Axon AI Functions Error",
			)
...
async def handle_ai_request_async(
	...
	try:
		...
		manager = AxonAgentManager(bot, file_handler=file_handler)
			...
			if should_fallback:
				...
				try:
					...
					if response and response.choices:
						...
						# Format the response
						from axon.ai.response_formatter import format_ai_response
