...
def get_action_defaults(action_id: str, message_id: str):
	...
	frappe.has_permission(doctype="Axon Message", doc=message_id, ptype="read", throw=True)
	action = frappe.get_doc("Axon Message Action", action_id)
	message = frappe.get_doc("Axon Message", message_id)

	# Loop through the fields in the action and get the default values from the message
	defaults = {}

	channel_doc = frappe.get_doc("Axon Channel", message.channel_id)
	workspace_id = channel_doc.workspace

	if not workspace_id:
		# Get the last workspace that this user has access to
		workspace_id = frappe.get_last_doc("Axon Workspace Member", {"user": frappe.session.user}).name

	url = frappe.utils.get_url(f"/axon/{workspace_id}")
