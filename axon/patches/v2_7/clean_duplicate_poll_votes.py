...
def execute():
	...
	single_select_duplicates = (
		...
		.where(axon_poll.is_multi_choice == 0)
	...
	for dup in single_select_duplicates:
		...
		votes = (
			...
			.where(axon_poll_vote.poll_id == dup.poll_id)
			.where(axon_poll_vote.user_id == dup.user_id)
			.orderby(axon_poll_vote.modified, order=Order.desc)
		).run(pluck="name")
	...
	from axon.axon_messaging.doctype.axon_poll_vote.axon_poll_vote import update_poll_votes
