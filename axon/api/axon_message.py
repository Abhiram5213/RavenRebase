...
from axon.api.axon_channel import create_direct_message_channel, get_peer_user_id
from axon.utils import get_channel_member, is_channel_member, track_channel_visit
...
def fetch_recent_files(channel_id: str):
	...
	"""
	if not frappe.has_permission("Axon Channel", doc=channel_id):
...
def save_message(message_id: str, add: str | bool = False):
	...
	if not frappe.has_permission(doctype="Axon Message", doc=message_id, ptype="read"):
	...
	toggle_like("Axon Message", message_id, add)

	liked_by = frappe.db.get_value("Axon Message", message_id, "_liked_by")
...
def get_pinned_messages(channel_id: str):
	...
	# check if the user has permission to view the channel
	frappe.has_permission("Axon Channel", doc=channel_id, ptype="read", throw=True)

	pinnedMessagesString = frappe.db.get_value("Axon Channel", channel_id, "pinned_messages_string")
...
def get_saved_messages():
	...
	axon_message = frappe.qb.DocType("Axon Message")
	axon_channel = frappe.qb.DocType("Axon Channel")
	axon_channel_member = frappe.qb.DocType("Axon Channel Member")

	query = (
		frappe.qb.from_(axon_message)
		.join(axon_channel, JoinType.left)
		.on(axon_message.channel_id == axon_channel.name)
		.join(axon_channel_member, JoinType.left)
		.on(axon_channel.name == axon_channel_member.channel_id)
		.select(
			axon_message.name,
			axon_message.owner,
			axon_message.creation,
			axon_message.text,
			axon_message.channel_id,
			axon_message.file,
			axon_message.message_type,
			axon_message.message_reactions,
			axon_message._liked_by,
			axon_channel.workspace,
			axon_message.thumbnail_width,
			axon_message.thumbnail_height,
			axon_message.is_bot_message,
			axon_message.bot,
		)
		.where(axon_message._liked_by.like("%" + frappe.session.user + "%"))
		.where(
			(axon_channel.type.isin(["Open", "Public"]))
			| (axon_channel_member.user_id == frappe.session.user)
		)
		.orderby(axon_message.creation, order=Order.asc)
		.distinct()
...
def check_permission(channel_id):
	if frappe.get_cached_value("Axon Channel", channel_id, "type") == "Private":
...
def get_unread_count_for_channels():
	...
	channel = frappe.qb.DocType("Axon Channel")
	channel_member = frappe.qb.DocType("Axon Channel Member")
	message = frappe.qb.DocType("Axon Message")
	query = (
...
def get_unread_count_for_channel(channel_id: str):
	...
	if channel_member:
		last_timestamp = frappe.get_cached_value(
			"Axon Channel Member", channel_member["name"], "last_visit"
		)
		...
		if frappe.get_cached_value("Axon Channel", channel_id, "type") == "Open":
			return frappe.db.count(
				"Axon Message",
				filters={
					"channel_id": channel_id,
...
def get_all_files_shared_in_channel(
	...
	message = frappe.qb.DocType("Axon Message")
	user = frappe.qb.DocType("Axon User")
