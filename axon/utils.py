...
def get_axon_room():
	"""
	Room which any user with the role "Axon User" is subscribed to.
	"""
...
def track_channel_visit(channel_id, user=None, commit=False, publish_event_for_user=False):
	...
	if channel_member:
		...
		frappe.db.set_value("Axon Channel Member", channel_member["name"], "last_visit", now)

	# Else if the user is not a member of the channel and the channel is open, create a new member record
	elif frappe.get_cached_value("Axon Channel", channel_id, "type") == "Open":
		frappe.get_doc(
			{
...
def get_workspace_members(workspace_id: str):
	...
	"""
	cache_key = f"axon:workspace_members:{workspace_id}"
...
def get_channel_members(channel_id: str):
	...
	"""
	cache_key = f"axon:channel_members:{channel_id}"
	...
	axon_user = frappe.qb.DocType("Axon User")

	query = (
		frappe.qb.from_(axon_channel_member)
		.join(axon_user)
		.on(axon_channel_member.user_id == axon_user.name)
		.select(
			axon_channel_member.name,
			axon_channel_member.user_id,
			axon_channel_member.is_admin,
			axon_channel_member.allow_notifications,
			axon_user.type,
		)
		.where(axon_channel_member.channel_id == channel_id)
		.where(axon_user.enabled == 1)
	)
...
def get_axon_user(user_id: str) -> str:
	"""
	Get the Axon User ID of a user
	"""
	# TODO: Run this via cache
	axon_user = frappe.qb.DocType("Axon User")
	query = (
		frappe.qb.from_(axon_user)
		.select(axon_user.name)
		.where((axon_user.user == user_id) | (axon_user.bot == user_id))
		.limit(1)
	)
...
def get_thread_reply_count(thread_id: str) -> int:
	...
	return frappe.cache().hget(
		"axon:thread_reply_count",
		thread_id,
		lambda: frappe.db.count(
			"Axon Message", {"channel_id": thread_id, "message_type": ["!=", "System"]}
		),
	)
...
def refresh_thread_reply_count(thread_id: str):
	...
	frappe.cache().hset("axon:thread_reply_count", thread_id, new_count)
...
def clear_thread_reply_count_cache(thread_id: str):
	...
	"""
	frappe.cache().hdel("axon:thread_reply_count", thread_id)
