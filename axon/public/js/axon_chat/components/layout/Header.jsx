...
const Header = ({
    ...
    return <div className="axon-header">
        {selectedChannel ? <ChannelHeader channelID={selectedChannel} onBackClick={closeChannel} /> :
            <div style={{
                ...
                <span role='button' className="axon-logo cal-sans" onClick={toggle}>axon</span>

                <a href='/axon' target='_blank' className='btn btn-xs open-axon-button' style={{
                    lineHeight: '1.4rem',
                    // width: "24px",
                    display: 'flex',
                    alignItems: 'center',
        ...
        <div>
            {unreadMessageCount ? <span className="axon-unread-count">{unreadMessageCount} unread</span> : null}
            <button className="btn btn-md back-button" onClick={toggleAxon}>
                {isOpen ?
...
const ChannelHeader = ({ channelID, onBackClick }) => {
    ...
    return <div className="axon-channel-header">
        ...
        {channel?.is_direct_message ? <DMChannelHeader channel={channel} /> :
            <a href={`/axon/channel/${channelID}`} target="_blank" title="Open channel in Axon">
                <ChannelIcon channelType={channel?.channel_type} />
                <span className="axon-channel-header-name cal-sans">{channel?.channel_name ?? channelID}</span>
            </a>
...
const DMChannelHeader = ({ channel }) => {
    ...
    return <a title="Open channel in Axon" className="axon-dm-channel-header" href={`/axon/channel/${channel.name}`} target="_blank">
        <Avatar user={user} fallback={channel.peer_user_id} />
        <span className="axon-channel-header-name cal-sans">{user?.full_name ?? user?.name}</span>
    </a>
