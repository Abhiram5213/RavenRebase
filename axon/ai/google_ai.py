...
def get_document_ai_processors():
	...
	"""
	frappe.has_permission("Axon Settings", ptype="read", throw=True)

	axon_settings = frappe.get_single("Axon Settings")
	if not axon_settings.enable_google_apis:
		return []

	location = axon_settings.google_processor_location

	key = json.loads(axon_settings.get_password("google_service_account_json_key"))
	...
	parent = client.common_location_path(axon_settings.google_project_id, location)
...
def run_document_ai_processor(processor_id: str, file_path: str, extension: str):
	...
	full_processor_name = client.processor_path(
		axon_settings.google_project_id, location, processor_id
	)
...
def create_document_processor(processor_type_key: str):
	...
	axon_settings = frappe.get_single("Axon Settings")
	if not axon_settings.enable_google_apis:
		frappe.throw(_("Google APIs are not enabled. Please enable them in the Axon Settings."))
	...
	display_name = f"Axon-{config['display_name']}"
...
def delete_document_processor(processor_id: str):
	...
	processor_name = client.processor_path(axon_settings.google_project_id, location, processor_id)
