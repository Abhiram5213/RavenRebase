...
if TYPE_CHECKING:
	from axon.axon_messaging.doctype.axon_message.axon_message import AxonMessage


class AxonIncomingWebhook(Document):
...
def handle_incoming_webhook(*args, **kwargs):
	...
	if not frappe.db.exists("Axon Incoming Webhook", webhook_id):
	...
	incoming_webhook: "AxonIncomingWebhook" = frappe.get_doc("Axon Incoming Webhook", webhook_id)  # type: ignore
	message: "AxonMessage" = frappe.new_doc("Axon Message")  # type: ignore
