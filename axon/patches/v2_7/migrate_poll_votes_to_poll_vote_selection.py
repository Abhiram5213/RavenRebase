...
def execute():
	...
	if not frappe.db.has_column("Axon Poll Vote", "option"):
		return

	axon_poll_vote = frappe.qb.DocType("Axon Poll Vote")
	axon_poll = frappe.qb.DocType("Axon Poll")
	...
	multi_select_duplicates = (
		frappe.qb.from_(axon_poll_vote)
		.join(axon_poll)
		.on(axon_poll_vote.poll_id == axon_poll.name)
		.select(
			axon_poll_vote.poll_id,
			axon_poll_vote.user_id,
		)
		.where(axon_poll.is_multi_choice == 1)
		.groupby(axon_poll_vote.poll_id, axon_poll_vote.user_id)
		.having(Count(axon_poll_vote.name) > 1)
	).run(as_dict=True)
	...
	for dup in multi_select_duplicates:
		...
		votes = (
			frappe.qb.from_(axon_poll_vote)
			.select(axon_poll_vote.name, axon_poll_vote.option)
			.where(axon_poll_vote.poll_id == dup.poll_id)
			.where(axon_poll_vote.user_id == dup.user_id)
			.orderby(axon_poll_vote.creation)
		).run(as_dict=True)
		...
		for idx, vote in enumerate(votes):
			if vote.option:
				frappe.get_doc(
					{
						"doctype": "Axon Poll Vote Selection",
						"parent": vote_to_keep.name,
						"parentfield": "vote_selection",
						"parenttype": "Axon Poll Vote",
						"idx": idx + 1,
						"option": vote.option,
					}
				).insert()
		...
		for vote in votes[1:]:
			frappe.delete_doc("Axon Poll Vote", vote.name, delete_permanently=True)

	votes_to_migrate = (
		frappe.qb.from_(axon_poll_vote)
		.select(
			axon_poll_vote.name,
			axon_poll_vote.option,
		)
		.where(axon_poll_vote.option.isnotnull())
		.where(axon_poll_vote.option != "")
	).run(as_dict=True)
	...
	for vote in votes_to_migrate:
		...
		existing = frappe.db.exists(
			"Axon Poll Vote Selection", {"parent": vote.name, "option": vote.option}
		)
		if not existing:
			frappe.get_doc(
				{
