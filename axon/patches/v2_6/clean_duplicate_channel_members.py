...
def execute():
	...
	query = (
		...
		.select(
			axon_channel_member.channel_id,
			axon_channel_member.user_id,
			Count(axon_channel_member.name).as_("count"),
		)
		.groupby(axon_channel_member.channel_id, axon_channel_member.user_id)
		.having(Count(axon_channel_member.name) > 1)
	)
	...
	frappe.db.sql(
		"""
        UPDATE `tabAxon Channel Member` rcm
        INNER JOIN (
            SELECT channel_id, user_id, MIN(creation) as min_creation
            FROM `tabAxon Channel Member`
        ...
        SET rcm.is_admin = 1
        WHERE EXISTS (
            SELECT 1 FROM `tabAxon Channel Member` dup
	...
	frappe.db.sql(
		"""
        DELETE rcm FROM `tabAxon Channel Member` rcm
        INNER JOIN (
            SELECT channel_id, user_id, MIN(creation) as min_creation
            FROM `tabAxon Channel Member`
            GROUP BY channel_id, user_id
