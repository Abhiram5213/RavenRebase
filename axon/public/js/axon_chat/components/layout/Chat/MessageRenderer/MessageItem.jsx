...
const MessageHeader = ({ message, user }) => {
    ...
    return <div className='axon-message-item-header'>
        <span className='axon-message-item-header-name'>{user?.full_name}</span>
        <span className='axon-message-item-header-time'>{moment(message.creation, frappe.defaultDatetimeFormat).format('HH:mm A')}</span>
    </div>
}
