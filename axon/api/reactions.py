...
from axon.utils import is_channel_member
...
def react(message_id: str, reaction: str, is_custom: bool = False, emoji_name: str = None):
	...
	if not frappe.has_permission("Axon Message", doc=message_id, ptype="read"):
		frappe.throw(_("You do not have permission to react to this message"), frappe.PermissionError)

	channel_id = frappe.get_cached_value("Axon Message", message_id, "channel_id")
	...
	try:
		...
		for fn in frappe.get_hooks("axon_message_reaction_after_delete"):
