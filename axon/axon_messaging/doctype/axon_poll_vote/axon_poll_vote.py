...
class AxonPollVote(Document):
	...
	if TYPE_CHECKING:
		...
		from axon.axon_messaging.doctype.axon_poll_vote_selection.axon_poll_vote_selection import (
			AxonPollVoteSelection,
		)

		poll_id: DF.Link
		user_id: DF.Link
		vote_selection: DF.Table[AxonPollVoteSelection]
	# end: auto-generated types

	def before_insert(self):
		# check if the poll is still open
		poll = frappe.get_cached_doc("Axon Poll", self.poll_id)
		if poll.is_disabled:
		...
		if frappe.db.exists("Axon Poll Vote", {"poll_id": self.poll_id, "user_id": self.user_id}):
...
def update_poll_votes(poll_id):
	...
	"""
	poll = frappe.get_doc("Axon Poll", poll_id, for_update=True)
	# get votes for each option
	poll_vote = frappe.qb.DocType("Axon Poll Vote")
	poll_vote_selection = frappe.qb.DocType("Axon Poll Vote Selection")
	...
	for option in poll.options:
		...
		frappe.db.set_value(
			"Axon Poll Option", option.name, "votes", option.votes, update_modified=False
		)

	# Count total unique voters (each Axon Poll Vote = 1 voter)
	total_votes = frappe.db.count("Axon Poll Vote", filters={"poll_id": poll_id})

	frappe.db.set_value("Axon Poll", poll_id, "total_votes", total_votes, update_modified=False)
