...
from axon.api.chat_stream import get_messages, get_newer_messages, get_older_messages

CHANNEL_ID = "Public Workspace-test-channel"

EXTRA_TEST_RECORD_DEPENDENCIES = ["Axon Workspace"]
...
class TestChatStream(IntegrationTestCase):
	def setUp(self):
		try:
			frappe.delete_doc("Axon Channel", CHANNEL_ID)
		except frappe.exceptions.DoesNotExistError:
			pass
		frappe.clear_cache(doctype="Axon Message")
		create_channel()
	...
	def test_get_messages_around_base_mid(self):
		...
		base_message_id = frappe.db.get_value(
			"Axon Message", {"text": "Test Message 50", "channel_id": CHANNEL_ID}, "name"
		)
	...
	def test_get_messages_around_base_with_fewer_new_messages(self):
		...
		"""
		base_message_id = frappe.db.get_value(
			"Axon Message", {"text": "Test Message 95", "channel_id": CHANNEL_ID}, "name"
		)
		response = get_messages(CHANNEL_ID, base_message=base_message_id)
	...
	def test_get_messages_around_base_with_fewer_old_messages(self):
		...
		"""
		base_message_id = frappe.db.get_value(
			"Axon Message", {"text": "Test Message 5", "channel_id": CHANNEL_ID}, "name"
		)
		response = get_messages(CHANNEL_ID, base_message=base_message_id)
