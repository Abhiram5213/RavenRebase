...
def create_axon_tools(bot) -> list[FunctionTool]:
	"""
	Create function tools for Axon bot

	Args:
	    bot: Axon bot document
	...
	if hasattr(bot, "bot_functions") and bot.bot_functions:
		...
		for func in bot.bot_functions:
			...
			try:
				# Get function details from Axon AI Function (not Axon Bot Functions)
				function_doc = frappe.get_doc("Axon AI Function", func.function)
				...
				if function_doc.type == "Custom Function":
					...
					if function_doc.type == "Get List":
						function_path = "axon.ai.sdk_tools.handle_get_list"
					elif function_doc.type == "Get Document":
						function_path = "axon.ai.sdk_tools.handle_get_document"
					elif function_doc.type == "Update Document":
						function_path = "axon.ai.sdk_tools.handle_update_document"
					elif function_doc.type == "Create Document":
						function_path = "axon.ai.sdk_tools.handle_create_document"
					elif function_doc.type == "Delete Document":
						function_path = "axon.ai.sdk_tools.handle_delete_document"
...
def create_function_tool(
	...
	"""
	Create a FunctionTool for Axon functions
