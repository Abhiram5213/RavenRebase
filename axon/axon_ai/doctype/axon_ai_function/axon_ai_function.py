...
class AxonAIFunction(Document):
	...
	if TYPE_CHECKING:
		...
		from axon.axon_ai.doctype.axon_ai_function_params.axon_ai_function_params import (
			AxonAIFunctionParams,
		)
		...
		parameters: DF.Table[AxonAIFunctionParams]
		params: DF.JSON | None
	...
	def on_update(self):
		...
		bots = frappe.get_all("Axon Bot Functions", filters={"function": self.name}, pluck="parent")

		for bot in bots:
			bot = frappe.get_doc("Axon Bot", bot)
