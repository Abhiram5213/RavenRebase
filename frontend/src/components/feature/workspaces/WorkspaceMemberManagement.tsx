...
import { AxonWorkspaceMember } from "@/types/Axon/AxonWorkspaceMember"
...
const RemoveMemberButton = ({ memberID, onUpdate }: { memberID: string, onUpdate: () => void }) => {
    ...
    const removeMember = () => {
        deleteDoc('Axon Workspace Member', memberID)
            .then(() => {
                onUpdate()
...
const MakeAdminButton = ({ memberID, onUpdate }: { memberID: string, onUpdate: () => void }) => {
    ...
    const makeAdmin = () => {
        updateDoc('Axon Workspace Member', memberID, { is_admin: true })
            .then(() => {
                onUpdate()
...
const RemoveAdminButton = ({ memberID, onUpdate }: { memberID: string, onUpdate: () => void }) => {
    ...
    const removeAdmin = () => {
        updateDoc('Axon Workspace Member', memberID, { is_admin: false })
            .then(() => {
                onUpdate()
                toast.success('Success')
            })
...
const ManageMembersDialogContent = ({ workspaceID, onClose, members }: { workspaceID: string, onClose: () => void, members: WorkspaceMemberFields[] }) => {
    ...
    const { call, loading, error } = useFrappePostCall('axon.api.workspaces.update_workspace_members')
