import { AxonBotFunctions } from '../AxonAI/AxonBotFunctions'
import { AxonAIBotFiles } from '../AxonAI/AxonAIBotFiles'

export interface AxonBot{
	creation: string
	name: string
	modified: string
	owner: string
...
File search enables the assistant with knowledge from files that you upload. Once a file is uploaded, the assistant automatically decides when to retrieve content based on user requests.	*/
	...
	/**	Bot Functions : Table - Axon Bot Functions	*/
	bot_functions?: AxonBotFunctions[]
	/**	OpenAI Vector Store ID : Data	*/
	openai_vector_store_id?: string
	/**	File Sources : Table - Axon AI Bot Files	*/
	file_sources?: AxonAIBotFiles[]
	/**	Use Google Document/Vision AI to parse documents : Check - When images or PDFs are uploaded to the agent, Axon will automatically call Google Cloud APIs to parse the document and send it's results to the agent.	*/
