      ...
      refresh: function (frm) {
        if (!frm.is_new()) {
          if (frm.footer?.frm?.timeline && !buttonAdded) {
            ...
            let get_channels = () => {
              return frappe.call({
                method: 'axon.api.axon_channel.get_channels',
                args: {
                  hide_archived: true,
                },
                callback: function (r) {
            ...
            // check the button is not already added by checking the class name 'send-axon-button'

            timeline.add_action_button(
              __('Send a Axon'),
              send_axon,
              'share',
              'btn-secondary send-axon-button'
            );
            buttonAdded = true;
          }
        }
