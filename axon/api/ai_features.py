...
from axon.ai.handler import get_variables_for_instructions
...
def get_instruction_preview(instruction: str):
	...
	"""
	frappe.has_permission(doctype="Axon Bot", ptype="write", throw=True)
...
def get_saved_prompts(bot: str = None):
	...
	prompts = frappe.get_list(
		"Axon Bot AI Prompt", or_filters=or_filters, fields=["name", "prompt", "is_global", "axon_bot"]
	)

	# Order by ones with the given bot
	prompts = sorted(prompts, key=lambda x: x.get("axon_bot") == bot, reverse=True)
...
def get_open_ai_version():
	...
	"""
	frappe.has_permission(doctype="Axon Bot", ptype="read", throw=True)
...
def get_openai_available_models():
	...
	"""
	frappe.has_permission(doctype="Axon Bot", ptype="read", throw=True)
	from axon.ai.openai_client import get_openai_models
...
def test_llm_configuration(
	...
	"""
	frappe.has_permission(doctype="Axon Settings", ptype="write", throw=True)
	...
	try:
		if provider == "Local LLM" and api_url:
			...
			from axon.ai.openai_client import get_open_ai_client
