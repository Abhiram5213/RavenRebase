...
def execute():
	...
	dm_channels = (
		frappe.qb.from_(axon_channel)
		.select(axon_channel.name)
		.where(axon_channel.is_direct_message == 1)
		.orderby(axon_channel.creation)
	).run(as_dict=True)
...
def merge_channel_into_primary(duplicate_channel: str, primary_channel: str):
	...
	frappe.db.delete("Axon Channel", {"name": duplicate_channel})
...
def migrate_linked_records(primary_channel: str, duplicate_channel: str):
	...
	linked_doctypes = [
		("Axon Message", "channel_id"),
		("Axon Message Reaction", "channel_id"),
		("Axon Pinned Channels", "channel_id"),
		("Axon Incoming Webhook", "channel_id"),
		("Axon Scheduler Event", "channel"),
		("Axon Scheduler Event", "dm"),
		("Axon Webhook", "channel_id"),
	]
...
def migrate_channel_members(primary_channel: str, duplicate_channel: str):
	...
	frappe.db.sql(
		"""
		UPDATE `tabAxon Channel Member`
		SET is_admin = 1
		WHERE channel_id = %s
		AND user_id IN (
			SELECT user_id FROM (
				SELECT user_id FROM `tabAxon Channel Member`
	...
	frappe.db.sql(
		...
		AND user_id NOT IN (
			SELECT user_id FROM (
				SELECT user_id FROM `tabAxon Channel Member` WHERE channel_id = %s
			) AS existing_members
		)
		""",
	...
	frappe.db.delete("Axon Channel Member", {"channel_id": duplicate_channel})
