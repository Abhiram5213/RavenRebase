...
from axon.api.axon_users import get_current_axon_user
from axon.utils import get_channel_members, get_axon_user, is_channel_member, track_channel_visit
...
def create_direct_message_channel(user_id: str):
	...
	# Validate both users are Axon Users
	if not get_axon_user(frappe.session.user):
		frappe.throw(_("You need to be a Axon User to send DMs."))

	if user_id != frappe.session.user:
		if not get_axon_user(user_id):
			frappe.throw(_("The user you are trying to message is not a Axon User."))
...
def toggle_pinned_channel(channel_id: str):
	...
	"""
	axon_user = get_current_axon_user()
	pinned_channels = axon_user.pinned_channels or []

	is_pinned = False
	for pin in pinned_channels:
		if pin.channel_id == channel_id:
			axon_user.remove(pin)
			is_pinned = True
			break

	if not is_pinned:
		axon_user.append("pinned_channels", {"channel_id": channel_id})
...
def leave_channel(channel_id: str):
	...
	for member in members:
		frappe.delete_doc("Axon Channel Member", member.name)
...
def toggle_pin_message(channel_id: str, message_id: str):
	...
	"""
	channel = frappe.get_doc("Axon Channel", channel_id)
	...
	message_exists = frappe.db.get_value("Axon Message", message_id, "channel_id") == channel_id
